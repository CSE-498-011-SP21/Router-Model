...
''' Only use input stream for this test '''
...
numpy.random.seed(3)
model.fit(y, target, nb_epoch=1, batch_size=1, verbose=2)

for i, result in enumerate(model.predict(y)):
	print(f"{usage[y[i]]}: {result}")
