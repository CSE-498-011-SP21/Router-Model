...
X, y = make_classification(n_features=1, n_redundant=0, n_informative=2,
...
for i, key in enumerate(usage):
    data[i] = np.array([key], dtype=np.int)
    target[i] = np.array([int(usage[key] > 4)], dtype=np.int)
    i += 1
# print(usage)
# print(data)
# print(target)

datasets = [(data, target)]
